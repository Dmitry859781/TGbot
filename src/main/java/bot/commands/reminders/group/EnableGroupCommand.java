...
public class EnableGroupCommand implements Command {
    ...
    public String getDescription() {
        return "Включить все напоминания в группе";
    }

    @Override
    public String getUsage() {
        return "/enableGroup <groupName>";
    }
    ...
    public void execute(TelegramBot bot, Message message, String[] args) {
        ...
        if (args.length == 0) {
            bot.sendMessage(chatId, "Укажите имя группы. Пример: /enableGroup Работа");
            return;
        }
        ...
        try {
            ...
            for (String name : reminderNames) {
                ...
                if (r != null && matchesGroup(r, groupName)) {
                    toggleReminderEnabled(r, true); // включить
                    count++;
                }
            }

            bot.sendMessage(chatId, count + " напоминаний в группе \"" + groupName + "\" включено.");
        } catch (Exception e) {
            e.printStackTrace();
            bot.sendMessage(chatId, "Ошибка при включении группы.");
        }
    }
